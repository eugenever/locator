    ...
    for chunk in data.chunks(chunk_size) {
        ...
        if let Err(e) = tx.execute(&query, &params).await {
            ...
            if let Some(pg_err) = e.as_db_error() {
                if pg_err.code().code() == "40P01" && _retries < MAX_RETRIES {
                    info!("deadlock detected, retrying... Attempt {}", _retries + 1,);
                    _retries = _retries + 1;
...
pub async fn _bulk_insert_async(pool: &PgPool, data: &[DataReport]) -> Result<(), PgError> {
